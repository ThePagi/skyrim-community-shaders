...
#	endif  // LANDSCAPE
		...
		float4 colorSample = TexColorSampler.Sample(SampColorSampler, diffuseUv);
		float landSnowMask1 = GetLandSnowMaskValue(colorSample.w);
		float4 normalSample = TexNormalSampler.Sample(SampNormalSampler, uv);
		float glossSample = 0;

#	if defined(MODELSPACENORMALS)
#		if defined(LODLANDNOISE)
		normalSample.xyz = normalSample.xzy - 0.5.xxx;
		float lodLandNoiseParameter = GetLodLandBlendParameter(colorSample.xyz);
		...
		colorSample.xyz *= lodLandNoiseMultiplier;
		normalSample.xyz *= 2;
		normalSample.w = 1;
		glossSample = 0;
#		elif defined(LODLANDSCAPE)
		normalSample.xyz = 2.0.xxx * (-0.5.xxx + normalSample.xzy);
		normalSample.w = 1;
		glossSample = 0;
#		else
		normalSample.xyz = normalSample.xzy * 2.0.xxx + -1.0.xxx;
		normalSample.w = 1;
		glossSample = TexSpecularSampler.Sample(SampSpecularSampler, uv).x;
#		endif  // LODLANDNOISE
#	elif (defined(SNOW) && defined(LANDSCAPE))
	normalSample.xyz = GetLandNormal(landSnowMask1, normalSample.xyz, uv, SampNormalSampler, TexNormalSampler);
	glossSample = normalSample.w;
#	else
	normalSample.xyz = TransformNormal(normalSample.xyz);
	glossSample = normalSample.w;
#	endif  // MODELSPACENORMALS

#	if defined(WORLD_MAP)
		normalSample.xyz = GetWorldMapNormal(input, normalSample.xyz, colorSample.xyz);  // TODO might require separate rawColorSample instead
#	endif                                                                               // WORLD_MAP

#	if defined(LANDSCAPE)
		landColors[0] = colorSample.rgb;
		landNormals[0] = normalSample.rgb;
		landGloss[0] = glossSample;
	}
#	else
	baseColor = colorSample;
	normal = normalSample;
	glossiness = glossSample;
...
#		endif  // CPM_AVAILABLE
		...
		landNormals[1] = landNormal2.xyz;
		landGloss[1] = landNormal2.w;
#		if defined(SNOW)
...
#		endif  // CPM_AVAILABLE
		...
		landNormals[2] = landNormal3.xyz;
		landGloss[2] = landNormal3.w;
#		if defined(SNOW)
...
#		endif  // CPM_AVAILABLE
		...
		landNormals[3] = landNormal4.xyz;
		landGloss[3] = landNormal4.w;
#		if defined(SNOW)
...
#		endif  // CPM_AVAILABLE
		...
		landNormals[4] = landNormal5.xyz;
		landGloss[4] = landNormal5.w;
#		if defined(SNOW)
...
#		endif  // CPM_AVAILABLE
		...
		landNormals[5] = landNormal6.xyz;
		landGloss[5] = landNormal6.w;
#		if defined(SNOW)
...
#		if defined(CPM_AVAILABLE)
	if (perPassParallax[0].EnableTerrainParallax) {
		float total = 0;
		for (int i = 0; i < 6; i++) {
			total += weights[i] * pixelOffset[i];
		}
		for (int i = 0; i < 6; ++i) {
			float w = weights[i] * pixelOffset[i] / total;
...
#	if defined(MULTI_LAYER_PARALLAX)
	...
	float3 layerNormal = MultiLayerParallaxData.yyy * (normalSample.xyz * 2.0.xxx + float3(-1, -1, -2)) + float3(0, 0, 1);
...
#endif  // PSHADER#include "Common/Color.hlsl"
#include "Common/FrameBuffer.hlsl"
#include "Common/GBuffer.hlsli"
#include "Common/LightingData.hlsl"
#include "Common/LodLandscape.hlsli"
